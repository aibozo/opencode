...
function applyUnifiedDiffText(diff: string, cwd: string) {
  ...
  while (i < lines.length) {
    ...
    if (!newHdr.startsWith("+++ b/")) { i += 1; continue }
    ...
    while (i < lines.length && lines[i].startsWith("@@")) {
      ...
      while (i < lines.length && !lines[i].startsWith("@@") && !lines[i].startsWith("--- a/")) {
        const L = lines[i]
        if (!L) { newBlock.push(""); i++; continue }
        const tag = L[0]
        const body = L.slice(1)
        if (tag === ' ') { newBlock.push(body) ; seen++ }
...
export function applyPatch(diffPath: string) {
