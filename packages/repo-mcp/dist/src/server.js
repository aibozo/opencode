...
const tools = [
    ...
    { name: "repo.session.start", description: "Start a metrics session; resets exposure log grouping.", inputSchema: ZSessionStartArgs.toJSON ? ZSessionStartArgs.toJSON() : { type: "object", properties: { label: { type: "string" } }, additionalProperties: false } },
    {
    ...
    { name: "repo.metrics.checkRecall", description: "Compute line-recall against git diff for this session.", inputSchema: ZRecallArgs.toJSON ? ZRecallArgs.toJSON() : { type: "object", properties: { sessionId: { type: "string" }, baseRef: { type: "string" }, headRef: { type: "string" } }, required: ["sessionId"], additionalProperties: false } },
    { name: "repo.frontier.expand", description: "Re-select working set with increased radius; for recall misses.", inputSchema: ZExpandArgs.toJSON ? ZExpandArgs.toJSON() : { type: "object", properties: { task: { type: "string" }, sessionId: { type: "string" }, radiusDelta: { type: "number" }, budgetTokens: { type: "number" } }, required: ["task", "sessionId"], additionalProperties: false } },
];
            ...
            if (req?.method === "tools/call") {
                ...
                try {
                    ...
                    if (name === "repo.workingSet.select") {
                        ...
                        if (sessionId)
                            logWorkingSet(sessionId, root, pack, fileLens);
                    ...
                    if (name === "repo.code.readSpan") {
                        ...
                        const sessionId = args?.sessionId || (process.env.SESSION_ID ?? "");
                        if (sessionId)
                            logSpan(sessionId, root, rel, p.start, p.end);
