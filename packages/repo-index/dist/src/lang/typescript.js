...
export function parseTypeScript(root, file) {
    ...
    if (Parser && TypeScript) {
        try {
            ...
            const exportFlag = (node) => { let n = node; while (n) {
                if (n.type?.includes?.("export"))
                    return true;
                n = n.parent;
            } return false; };
            const walk = (node, classCtx) => {
                if (node.type === "function_declaration" || node.type === "method_definition") {
                    ...
                    const kind = node.type === "method_definition" ? "method" : "func";
                    ...
                    for (const idn of ids)
                        edges.push({ from: full, to: idn.text, type: "calls" });
                }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (mFunc)
            symbols.push({ file, lang: "typescript", name: scopedName([mFunc[2]]), shortName: mFunc[2], kind: "func", startLine: i + 1, endLine: i + 1, export: !!mFunc[1] });
        const mClass = /^\s*(export\s+)?class\s+([A-Za-z_][\w]*)/.exec(l);
        if (mClass)
            symbols.push({ file, lang: "typescript", name: scopedName([mClass[2]]), shortName: mClass[2], kind: "class", startLine: i + 1, endLine: i + 1, export: !!mClass[1] });
