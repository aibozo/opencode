...
export function parsePython(root, file) {
    ...
    if (Parser && Python) {
        try {
            ...
            const docstringFor = (node) => {
                const suite = node?.namedChildren?.find((n) => n.type === "block" || n.type === "suite") ?? node;
                const first = suite?.namedChildren?.find((n) => n.type === "expression_statement");
            ...
            const walk = (node, classCtx) => {
                if (node.type === "function_definition") {
                    ...
                    const ids = body?.descendantsOfType?.(["identifier"]) ?? [];
                    for (const id of ids)
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (mFunc)
            symbols.push({ file, lang: "python", name: scopedName([mFunc[1]]), shortName: mFunc[1], kind: "func", startLine: i + 1, endLine: i + 2 });
        const mClass = /^\s*class\s+([A-Za-z_][\w]*)/.exec(line);
        if (mClass)
            symbols.push({ file, lang: "python", name: scopedName([mClass[1]]), shortName: mClass[1], kind: "class", startLine: i + 1, endLine: i + 2 });
        if (/^\s*import\b/.test(line))
            edges.push({ from: scopedName([]), to: line.trim(), type: "imports" });
        const callRe = /\b([A-Za-z_][\w]*)\s*\(/g;
        let m = null;
        while ((m = callRe.exec(line)) !== null) {
            const name = m[1];
            if (/(def|class|if|for|while|with|return|print|assert|lambda)$/.test(name))
                continue;
