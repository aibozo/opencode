...
export function skeletonizePython(root, file, src) {
    ...
    rootNode.namedChildren?.forEach((n) => {
        if (n.type === "import_statement" || n.type === "import_from_statement")
            out.push(lines[n.startPosition.row]);
    });
    if (out.length)
        out.push("");
    function emit(node, ind, ctx) {
        ...
        if (node.type === "class_definition") {
            ...
            const f = suite?.namedChildren?.find((n) => n.type === "expression_statement");
            const lit = f?.namedChildren?.[0];
            if (lit && (lit.type === "string" || lit.type === "string_literal"))
                out.push(ind + "    " + src.slice(lit.startIndex, lit.endIndex));
            node.namedChildren?.forEach((ch) => { if (ch.type === "function_definition")
                emit(ch, ind + "    ", name); });
            const body = node.childForFieldName("body");
            if (body) {
