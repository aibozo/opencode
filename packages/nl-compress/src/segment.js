export function segment(text) {
    ...
    while (i < ls.length) {
        ...
        if (/(^Traceback|^\s*at\s|File ".*", line \d+)/.test(ls[i])) {
            const a = i;
            let b = i;
            while (b + 1 < ls.length && /(^\s+at\s|^\s*File ".*", line \d+|^\s+.*Error|^\s*Traceback|^\s*Caused by:)/.test(ls[b + 1]))
                b++;
        ...
        const a = i;
        while (i + 1 < ls.length && ls[i + 1].trim() !== "")
            i++;
        const b = i;
        ...
        if (blk.length) {
            if (blk.length <= 3) {
                const t = blk.join("\n").trim();
                if (t)
                    segs.push({ text: t, start: a, end: b, kind: "para" });
            }
            else {
                let k = 0;
                while (k < blk.length) {
                    ...
                    const t = sub.join("\n").trim();
                    if (t)
                        segs.push({ text: t, start: a + k, end: Math.min(a + k + 2, b), kind: "para" });
                    k += 3;
                }
            }
        }
