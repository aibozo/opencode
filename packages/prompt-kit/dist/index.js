...
export function stablePrefixBlock() {
    ...
    for (const f of pack.full) {
        if (f.ranges && f.ranges.length) {
            for (const r of f.ranges)
                out.push(`- ${f.path} [L${r[0]}-${r[1]}]`);
        }
        else
            out.push(`- ${f.path} [FULL]`);
    }
    out.push("", "## SKELETON");
    for (const s of pack.skeletons)
        out.push(`- ${s.path} (elided; sha1=${((s.hash ?? "").slice(0, 7))}…)`);
    if (pack.interfaces && pack.interfaces.length) {
        out.push("", "## INTERFACES");
        for (const i of pack.interfaces)
            out.push(`- ${i.path}`);
    }
    if (pack.search_hits && pack.search_hits.length) {
        out.push("", "## SEARCH HITS (top)");
        for (const h of pack.search_hits.slice(0, 10))
            out.push(`- ${h.path}:${h.line} ${h.text}`);
    }
    return out.join("\n");
}
export function assemblePrompt(pack, notes, plan) {
